...
public partial class GeometryTrailEffectRenderSystem : SystemBase
{
    ...
    )
    {
        ...
        if (!geometryTrailEffectViewComponent.MeshUpdated && !geometryTrailEffectViewComponent.FadeTrail)
            return;

        if (geometryTrailEffectViewComponent.FadeTrail)
            for (var i = 0; i < trailEffectBuffer.Length; i++)
            {
                if (trailEffectBuffer[i].Lifetime > 0f)
                {
                    trailEffectBuffer[i] = new GeometryTrailEffectInfoData()
                    { 
        ...
        for (var i = 0; i < trailEffectBuffer.Length - 1; i++)
        {
            ...
            if (geometryTrailEffectViewComponent.StartFadePoints>0)
             startFadeValue = Mathf.Clamp01(startCounter / geometryTrailEffectViewComponent.StartFadePoints);
            ...
            verticies[counter * 2] = startPoint.Point1_Lt;
                ...
                colors[i * 2].a = startPoint.Lifetime / geometryTrailEffectViewComponent.TrailLifetime * startFadeValue;
                colors[i * 2+1].a = startPoint.Lifetime / geometryTrailEffectViewComponent.TrailLifetime * startFadeValue;
                colors[i * 2+2].a = startPoint.Lifetime / geometryTrailEffectViewComponent.TrailLifetime * startFadeValue;
                colors[i * 2+3].a = startPoint.Lifetime / geometryTrailEffectViewComponent.TrailLifetime * startFadeValue;
        ...
        geometryTrailEffectViewComponent.MeshUpdated = false;
    }
}
