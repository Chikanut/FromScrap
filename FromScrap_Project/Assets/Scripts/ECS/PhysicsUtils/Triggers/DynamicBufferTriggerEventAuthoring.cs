...
namespace Unity.Physics.Stateful
{
    ...
    public partial class TriggerEventConversionSystem : SystemBase
    {
        ...
        {
            (m_PreviousFrameTriggerEvents, m_CurrentFrameTriggerEvents) = (m_CurrentFrameTriggerEvents, m_PreviousFrameTriggerEvents);
        ...
        protected override void OnUpdate()
        {
            ...
            var colliderPairs = GetBufferFromEntity<PhysicsColliderKeyEntityPair>(true);
            
            var collectTriggerEventsJob = new CollectTriggerEventsJob
            {
                ColliderPairs = colliderPairs,
        ...
        public struct CollectTriggerEventsJob : ITriggerEventsJob
        {
            ...
            {
                var entityA = UpdateEntityWithChilds(triggerEvent.EntityA, triggerEvent.ColliderKeyA.Value);
                var entityB = UpdateEntityWithChilds(triggerEvent.EntityB, triggerEvent.ColliderKeyB.Value);
                
                TriggerEvents.Add(new StatefulTriggerEvent(
                    entityA, entityB, triggerEvent.BodyIndexA, triggerEvent.BodyIndexB,
            ...
            Entity UpdateEntityWithChilds(Entity entity, uint colliderKey)
            {
                ...
                var colliderPair = ColliderPairs[entity];
                for (int i = 0; i < colliderPair.Length; i++)
                {
                    if (colliderPair[i].Key.Value == colliderKey)
                    {
                        return colliderPair[i].Entity;
                    }
                }
