...
void Outline_float(float2 UV, float OutlineThickness, float DepthSensitivity, float ColorSensitivity, float MaxOutlineDistance, float4 OutlineColor, out float4 Out)
{
    ...
    float edgeDepth = sqrt(pow(depthFiniteDifference0, 2) + pow(depthFiniteDifference1, 2)) * 100 * MaxOutlineDistance;
