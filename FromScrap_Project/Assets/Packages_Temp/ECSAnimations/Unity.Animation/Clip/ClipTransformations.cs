...
namespace Unity.Animation
{
    public static class ClipTransformations
    {
        ...
        [BurstCompatible(GenericTypeArguments = new[] { typeof(int) })]
        private static void Intersection<T>(ref BlobArray<T> a, ref BlobArray<T> b, ref NativeList<T> result) where T : unmanaged, IEquatable<T>
        {
            result.Clear();
        ...
        [BurstCompatible(GenericTypeArguments = new[] { typeof(int) })]
        private static unsafe void SetBlobArray<T>(ref BlobBuilder builder, ref BlobArray<T> a, ref NativeList<T> result, out BlobBuilderArray<T> blobArray) where T : unmanaged
        {
