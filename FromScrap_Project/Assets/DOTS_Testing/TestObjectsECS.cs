...
namespace DOTS_Test
{
    ...
    {
        [Header("Settings")] [SerializeField] private int _objectsCount;
        ...
        [Header("Resources")] [SerializeField] private Mesh _mesh;
        ...
        void SpawnPhysicsEntities()
        {
            ...
            for (int i = 0; i < _objectsCount; i++)
            {
                ...
                {
                    Value = new float3(_spawnRectPos.x + Random.Range(-_spawnRect.x / 2, _spawnRect.x / 2),
                        _spawnRectPos.y + Random.Range(-10, 25),
                        _spawnRectPos.z + Random.Range(-_spawnRect.y / 2, _spawnRect.y / 2))
                });
            }
        }
        ...
        void SpawnRenderEntities()
        {
            ...
            for (int i = 0; i < entityArray.Length; i++)
            {
                ...
                {
                    Value = new float3(_spawnRectPos.x + Random.Range(-_spawnRect.x / 2, _spawnRect.x / 2),
                        _spawnRectPos.y, _spawnRectPos.z + Random.Range(-_spawnRect.y / 2, _spawnRect.y / 2))
                });
                entityManager.AddComponentData(entity, new CircleColliderComponent() {Radius = 0.5f});
                entityManager.AddComponentData(entity, new MovementComponent()
                {
                    Velocity = new float3(Random.Range(_movementSpeedRange.x, _movementSpeedRange.y), 0,
                        Random.Range(_movementSpeedRange.x, _movementSpeedRange.y))
